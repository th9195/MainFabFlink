...
object FlinkStreamEnv {
  ...
  def init(jobName: String) = {
    ...
    jobName match {
      ...
      case VmcConstants.VmcWriteIndicatorJob =>{
        env.setParallelism(ProjectConfig.SET_PARALLELISM_VMC_WRITE_INDICATOR_JOB)

        setCheckpoint(env,
          isEnable = ProjectConfig.CHECKPOINT_ENABLE_VMC_WRITE_INDICATOR_JOB,
          stateBackend = ProjectConfig.CHECKPOINT_STATE_BACKEND_VMC_WRITE_INDICATOR_JOB,
          checkpointInterval = ProjectConfig.CHECKPOINT_INTERVAL_VMC_WRITE_INDICATOR_JOB,
          checkpointTimeout = ProjectConfig.CHECKPOINT_TIME_OUT_VMC_WRITE_INDICATOR_JOB)
      }
