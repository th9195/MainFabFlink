...
import com.hzw.fdc.scalabean.VmcBeans._
...
class VmcAllAddStepProcessFunction() extends KeyedProcessFunction[String, JsonNode,  JsonNode] {
  ...
  // 记录eventStart
  var vmcEventStartDataMatchControlPlanState: ValueState[JsonNode] = _
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    vmcEventStartDataMatchControlPlanState = getRuntimeContext.getState(vmcEventDataMatchControlPlanStateDescription)
  ...
  override def processElement(inputValue: JsonNode, context: KeyedProcessFunction[String, JsonNode, JsonNode]#Context, collector: Collector[JsonNode]): Unit = {
    try{
      ...
      case e:Exception => {
        logger.error(s"添加 step 算子 processElement error！inputValue == ${inputValue}\n " +
          s"${e.printStackTrace()}")
      }
    }
  }
  ...
  def processEventStart(inputValue: JsonNode, context: KeyedProcessFunction[String, JsonNode, JsonNode]#Context, collector: Collector[JsonNode]) = {
    try{
      ...
      updateVmcEventStartDataMatchedControlPlan(inputValue)
    }catch {
      case e:Exception => {
        logger.error(s"添加 step 算子 processEventStart error ! inputValue == ${inputValue}\n " +
          s"${e.printStackTrace()}")
      }
    }
  ...
  def processEventEnd(inputValue: JsonNode, context: KeyedProcessFunction[String, JsonNode, JsonNode]#Context, collector: Collector[JsonNode]) = {
    try{
      ...
      case e:Exception => {
        logger.error(s"添加 step 算子 processEventEnd error ! inputValue == ${inputValue}\n " +
          s"${e.printStackTrace()}")
      }
    }
  }
  ...
  def processRawData(inputValue: JsonNode, context: KeyedProcessFunction[String, JsonNode, JsonNode]#Context, collector: Collector[JsonNode]) = {
    try{
      ...
      case e:Exception => {
        logger.error(s"添加 step 算子 processRawData error ! inputValue == ${inputValue}\n " +
          s"${e.printStackTrace()}")
      }
    }
  }
   ...
   * @param inputValue
   */
  def updateVmcEventStartDataMatchedControlPlan(inputValue: JsonNode) = {
    vmcEventStartDataMatchControlPlanState.update(inputValue)
  }
  ...
  def updateVmcControlPlanConfigSensorListState(inputValue: JsonNode) = {
    ...
    vmcControlPlanConfig.vmcSensorConfigMap.keys.foreach(sensorName => {
      vmcControlPlanConfigSensorListState.add(sensorName)
    })
  }
  ...
  def updateVmcRawDataStepIdListState(currentStepId: Long): Boolean = {
    ...
    logger.warn(s"stepIdStr == ${stepIdStr} ; currentStepId == ${currentStepId}")
    if(null == stepIdStr){
      vmcRawDataStepIdListState.update(currentStepId.toString)
      true
    }else if(!stepIdStr.split("\\|").toList.contains(currentStepId.toString)){
  ...
  def addStep(inputValue: JsonNode,
    ...
    try {
      ...
      // 获取sensorDataList
      val vmcSensorCacheInfoList: List[VmcSensorCacheInfo] = getSensorListData(vmcRawDataMatchedControlPlan, logger)

      // 生成VmcRawDataAddStep
      val stepId = stepIdName._1
      val stepName = stepIdName._2
      logger.warn(s"stepId == ${stepId} ; stepName == ${stepName}")
      val vmcRawDataAddStep: VmcRawDataAddStep = generateVmcRawDataAddStep(vmcRawDataMatchedControlPlan, stepId, stepName, vmcSensorCacheInfoList)
  ...
  def updateVmcRawDataCurrentStepIdState(stepID: Long) = {
    vmcRawDataCurrentStepIdState.update(stepID)
  }
  ...
  def getStepIdAndStepName(vmcRawDataMatchedControlPlan:VmcRawDataMatchedControlPlan) = {
    ...
    logger.error(s"traceId == ${vmcRawDataMatchedControlPlan.traceId} ; lastStepId == ${lastStepId}")
    ...
    updateVmcRawDataCurrentStepIdState(stepID)
    (stepID,stepName)
  }
...
//          unit = one.unit)
        VmcSensorCacheInfo(sensorAlias = one.sensorAlias,
          sensorValue = one.sensorValue.toString.toDouble,
          unit = one.unit)
      } catch {
        case exception: Exception =>
   ...
   * @return
   */
  def generateVmcRawDataAddStep(vmcRawDataMatchedControlPlan: VmcRawDataMatchedControlPlan, stepID: Long, StepName: String, vmcSensorCacheInfoList: List[VmcSensorCacheInfo]): VmcRawDataAddStep = {
    VmcRawDataAddStep(dataType = vmcRawDataMatchedControlPlan.dataType,
      ...
      vmcSensorCacheInfoList = vmcSensorCacheInfoList,
  ...
  def collectEventStart(currentStepId: Long, collector: Collector[JsonNode]) = {
    ...
    val eventStartJson: JsonNode = vmcEventStartDataMatchControlPlanState.value()
