...
import com.hzw.fdc.function.online.vmc.all.{VmcAllAddStepProcessFunction, VmcAllCalcIndicatorProcessFunction, VmcAllEtlProcessFunction, VmcAllMatchControlPlanProcessFunction}
      ...
      addStepDataTream.addSink(new FlinkKafkaProducer(
        ...
        new FdcKafkaSchema[JsonNode](ProjectConfig.KAFKA_VMC_ADD_STEP_TOPIC
          ...
          , (elem: JsonNode) => {
            ...
            s"${traceId}_${controlPlanId}_${stepId}"
          }
        )
        , ProjectConfig.getKafkaProperties()
        , FlinkKafkaProducer.Semantic.AT_LEAST_ONCE
        ...
        //添加uid用于监控
        .uid("addStepDataTream dataStream sink to kafka")
    }

    val vmcIndicatorDataTream = addStepDataTream.keyBy(inputValue => {
      val traceId = inputValue.findPath(VmcConstants.TRACE_ID).asText("-1")
      val controlPlanId = inputValue.findPath(VmcConstants.CONTROLPLAN_ID).asText("-1")
      val stepId = inputValue.findPath(VmcConstants.STEP_ID).asText("-1")
      s"${traceId}_${controlPlanId}_${stepId}"
    }).process(new VmcAllCalcIndicatorProcessFunction)
      .name("vmc all calc indicator ")
      .uid("vmc all calc indicator ")

    vmcIndicatorDataTream.addSink(new FlinkKafkaProducer(
      ProjectConfig.KAFKA_VMC_INDICATOR_TOPIC,
      new FdcKafkaSchema[JsonNode](ProjectConfig.KAFKA_VMC_INDICATOR_TOPIC
        //按照tool分区
        , (elem: JsonNode) => {
          val toolName = elem.findPath(VmcConstants.TOOL_NAME).asText("-1")
          toolName
        }
      )
      , ProjectConfig.getKafkaProperties()
      , FlinkKafkaProducer.Semantic.AT_LEAST_ONCE
    )).name("indicator data sink to kafka")
      //添加uid用于监控
      .uid("indicator data sink to kafka")
